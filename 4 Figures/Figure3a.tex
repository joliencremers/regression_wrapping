...
\begin{document}
	...
	% scale=0.8,
	declare function={
		wc(\x) = 1 + 1/(2*pi)*(1-0.1*0.1)/(1+0.1*0.1-2*0.1*cos(deg(\x-0.5*pi))); %wc(mu=0.5pi, rho=0.5) 
		wn(\x)= 1 + 
		(1/sqrt(2*pi*4.6)*exp(-1/(2*4.6)*(\x-0.5*pi)*(\x-0.5*pi))) +     (1/sqrt(2*pi*4.6)*exp(-1/(2*4.6)*(\x-2.5*pi)*(\x-2.5*pi))) +
		(1/sqrt(2*pi*4.6)*exp(-1/(2*4.6)*(\x+1.5*pi)*(\x+1.5*pi))); %wrappedN(mu=0.5pi, sd=2)
	}
	]	
